...
def get_scheduler(optimizer, opt):
    ...
    if opt.TRAIN.lr_policy == 'linear':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + opt.TRAIN.epoch_begin - opt.TRAIN.niter) / float(opt.TRAIN.niter_decay + 1)
            return lr_l
        ...
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt.TRAIN.niter, eta_min=0)
    else:
