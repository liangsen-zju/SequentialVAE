...
class VRNN(nn.Module):
    ...
    def generate(self, nsteps, device, nbatch=1):
        ...
        for t in range(nsteps):
            ...
            # prior   
            pmu, plogvar = self.Prior(self.feat_h)                 # B x dim_h

            # decoder
            z = self.Reparameterize(pmu, plogvar)

            feat_z = self.encoder_z(z)
            pred = self.Decoder(feat_z, self.feat_h)               # B x 1 x dim_x
    ...
    def forward(self, x):
        ...
        for t in range(nsteps):
            ...
            debug(self.opt, f"[Forward] feat_x = {feat_x.shape}", verbose=False) 
            
            # prior
            pmu, plogvar = self.Prior(self.feat_h)                 # B x dim_h
            xmu, xlogvar = self.Encoder(feat_x, self.feat_h)       # B x dim_h
    ...
    def Prior(self, feat_h):
        ...
        feat = self.encoder_p(feat_h)
        debug(self.opt, f"[PriorNet] encoder shape = {feat.shape}")
    ...
    def Encoder(self, feat_x, feat_h):
        """Encoder function.
        Args:
            x:,     [B x 1 x dim_h]
        ...
        """
        # concate with hidden
        feat = torch.cat([feat_x, feat_h], dim=-1)        # B x 1 x 2*dim_h
    ...
    def Decoder(self, feat_z,  feat_h):
        ...
        # concate with hidden
        feat = torch.cat([feat_z, feat_h], dim=-1)               # B x 1 x 2*dim_h
    ...
    def UpdateNet(self, feat_x, feat_z, hidden):
        ...
        """
        feat = torch.cat([feat_x, feat_z], dim=-1)         # (B, 1, 2*dim_h)
