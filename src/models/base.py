...
class Base():
    ...
    def setup(self, opt):
        ...
        if self.isTrain:
            ...
            if opt.TRAIN.resume:
                ...
                load_suffix = 'iter_%d' % opt.TRAIN.load_iter if opt.TRAIN.load_iter > 0 else opt.TRAIN.epoch_begin
            ...
            # load model for pretrain
                ...
                load_suffix = 'iter_%d' % opt.MODEL.load_iter if opt.MODEL.load_iter > 0 else opt.MODEL.load_epoch
        ...
        # if test or infer
            ...
            load_suffix = 'iter_%d' % opt.MODEL.load_iter if opt.MODEL.load_iter > 0 else opt.MODEL.load_epoch
            path_chekpoints = Path(opt.MODEL.path_checkpoint)
            self.load_networks(load_suffix, path_chekpoints)
    ...
    def load_networks(self, load_suffix, load_path, load_info=False):
        ...
        for name in self.names_model:
            ...
            state_dict = torch.load(str(path_checkpoint), map_location=str(self.device))
        ...
        if load_info:
            ...
            state_dict = torch.load(str(path_state))
            
            for name in self.names_model:
                optimizer_ = getattr(self, 'optimizer_' + name)
                scheduler_ = getattr(self, 'scheduler_' + name)
                optimizer_.load_state_dict(state_dict['optimizers']['optimizer_' + name])
                scheduler_.load_state_dict(state_dict['schedulers']['scheduler_' + name])
